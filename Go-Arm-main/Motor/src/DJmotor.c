...
/* 按 MOTOR_DJI_CAN_BUS 取总线句柄(0=FDCAN1,1=FDCAN2,2=FDCAN3) */
static inline FDCAN_HandleTypeDef *DJmotor_GetCanHandle(void)
{
...
void DJmotor_Init(void)
{
    ...
    dj3508_param.CurrentLimit_raw = 14000;
    ...
    for (uint32_t i = 0; i < USE_DJNUM; i++)
    {
        ...
        PID_Init(&DJmotor[i].velPID, 5.5f, 0.25f, 0.01f, PIDINC);
    }
}
