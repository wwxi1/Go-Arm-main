...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  Bsp_BoardInit();
#if APP_START_BUZZER_ON_BOOT
  BspBuzzer_StartupBeep();
#endif
  CAN_InitSendQueue();
  CAN_Start();
#if USE_DJ
  ...
  /* 各串口空闲 DMA 接收启动(USART2 未配 DMA,需要的话在 CubeMX 里加) */
  // HAL_UARTEx_ReceiveToIdle_DMA(&huart1, UART1_RxBuffer, UART_RX_BUFFER_SIZE);
  SolenoidValve_Init(Solenoid_Channel1);
...
void MPU_Config(void)
{
  ...
  MPU_InitStruct.SubRegionDisable = 0xFA;
